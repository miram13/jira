...
export default function PostJira() {
  const status = ['Active', 'Progress','Pending','Completed'];
  const group = ['RHEL','MW','OS','OCP','IAM'];
  const [summary, setSummary] = useState("");
  const [label, setLabel] = useState("");
  const [issue, setIssue] = useState("");
  ...
  const handleSubmit = () => {
    console.log(summary,label,issue, state);
    axios
      .post('http://localhost:8080/Jira/jira', {
        issue:issue,
        summary:summary,
                ...
                <DialogContent>
                  ...
                  <InputLabel id="demo-simple-select-label">Label</InputLabel>
                    <Select 
                     ...
                     value={label}
                     label="Label"
                    variant="standard"  autoFocus fullWidth onChange={(e) => setLabel(e.target.value)}>
                    
                      <option >Select Label</option>
                      
                      {group.map(option => <option key={option} value={option}>{option}</option>)}
                    </Select>
                  ...
                  <InputLabel id="demo-simple-select-label">State</InputLabel>
                    <Select 
                     ...
                     value={state}
                     label="State"
                    variant="standard"  autoFocus fullWidth onChange={(e) => setState(e.target.value)}>
